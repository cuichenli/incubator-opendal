...
impl Accessor for GcsBackend {
    ...
    async fn write(&self, path: &str, args: OpWrite) -> Result<(RpWrite, Self::Writer)> {
        let upload_location = if args.append() {
            ...
            match status {
                StatusCode::OK => {
                    let bs = resp.headers().into_iter().filter(|&x| x.0.as_str().eq("Location")).next().expect("Failed to retrieve location of resumable upload");
                    Some(String::from(bs.1.as_bytes()))
                }
...
impl GcsBackend {
    ...
    async fn gcs_initiate_resumable_upload(
        ...
        let p = build_abs_path(&self.root, path);
        let url = format!("{}/upload/storage/v1/b/{}/o?uploadType=resumable&name={}", self.endpoint, self.bucket, p);
        let mut req = Request::post(&url)
            .body(AsyncBody::Empty)
            .map_err(new_request_build_error)?;
        self.signer.sign(&mut req).map_err(new_request_sign_error)?;
    ...
    pub fn gcs_upload_chunks_in_resumable_upload(
        ...
        let rangeHeader = if is_last_chunk {
            format!("bytes {}-{}/{}", already_sent_bytes, already_sent_bytes + size -1, already_sent_bytes + size)
        } else {
            format!("bytes {}-{}/*", already_sent_bytes , already_sent_bytes + size - 1)
        };

        req = req.header(
            CONTENT_LENGTH, size).header(CONTENT_RANGE, rangeHeader);
    ...
    pub async fn gcs_complete_resumable_upload(
        ...
        let mut req = Request::post(upload_location)
            ...
            .header(CONTENT_RANGE, format!("bytes {}-{}/{}", already_uploaded_chunk - 1, already_uploaded_chunk - 1, already_uploaded_chunk))
            .body(AsyncBody::Empty)
            .map_err(new_request_build_error)?;
